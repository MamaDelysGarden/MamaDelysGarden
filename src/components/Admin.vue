<template lang="pug">
  q-layout
    q-toolbar(slot="header" reveal)
      ...
      q-btn(color="secondary" style="width: 100px", @click="$router.push({name: 'index'})") Home
...
export default {
  ...
  methods: {
    async login(e, done) {
      try {
        if(!this.user.email || !this.user.password) throw new Error('Error')
        ...
        this.$router.push({ name: 'admin-products' })
          ...
          Toast.create.negative({html: 'Error Logging in'})
      } finally {
        done()
      }
    },
